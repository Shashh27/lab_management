...
import { HomeOutlined, FileDoneOutlined, FileSearchOutlined , LogoutOutlined , ArrowLeftOutlined, DeleteOutlined , CloseCircleOutlined , CheckCircleOutlined } from '@ant-design/icons';
      ...
      sections: [{
        blocks: [
          ...
          },
          {
            paragraphFormat: {},
            inlines: [{ 
              text: 'Transducer size and frequency : \n',
              characterFormat: { bold: true, size: 14 }
            }]
          },
          {
...
export default function Authorizer() {
  ...
  const [TaskShowEditor, setTaskShowEditor] = useState(false);
  ...
  const handleStatusChange = (index, status) => {
    const task = taskData[index]; // Use the index to find the task
    if (!task) return;
  ...
  const handleDeleteTask = async (index) => {
    const task = taskData[index]; // Get the task data using index
    const { dc_po_number, report, technician } = task;
  
    try {
      const response = await axios.delete('http://localhost:3000/delete-tasks', {
        data: { dc_po_number, report, technician }, // Pass data in request body
      });
      ...
      message.error('Error occurred while deleting the task.');
    }
  };
    ...
    { title: 'Authorizer Status', dataIndex: 'authorizer_status', key: 'authorizer_status' ,align:'center',
     ...
     },
     {
      ...
      render: (_, record, index) => (
        // Use index from render function
        <Space>
          <Button
            style={{ borderColor: "#34A853", color: '#34A853' }}
            type="default"
            icon={<CheckCircleOutlined />}
            onClick={() => handleStatusChange(index, 'Approved')} // Pass index
          >
            Approve
          </Button>
          <Button
            ...
            onClick={() => handleStatusChange(index, 'Rejected')} // Pass index
          >
            Reject
          </Button>
        </Space>
  ...
  try {
    const selectedTemplateData = DOCUMENT_TEMPLATES.find(
      (template) => template.id === selectedTemplate
    );

    // Create initial content in SFDT format
    const initialContent = {
      sections: [{
        blocks: [
          {
            paragraphFormat: { textAlignment: 'Left' },
            inlines: [{ text: newDocTitle, characterFormat: { bold: true, size: 16 } }]
          },
          ...(selectedTemplateData?.content?.sections?.[0]?.blocks || [])
        ]
      }]
    };
              ...
              TaskShowEditor ? (
                <div>
                   <Button
                    ...
                    onClick={() => setTaskShowEditor(false)}
